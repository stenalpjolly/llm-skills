...
## Rules
1. **Never build from scratch manually.** To avoid import mistakes, always execute `python3 scripts/scaffold_ng_component.py --name {component_name} --out {dest_dir}` first to generate perfect, syntactically correct Angular standalone boilerplate.
...
2. Run the scaffolding script:
   ```bash
   python3 scripts/scaffold_ng_component.py --name {ComponentName} --out {dest_dir}
   ```
