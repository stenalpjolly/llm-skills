...
## Rules
1. **Never manual map TypeScript interfaces.** To avoid schema compilation errors, always run `python3 ../shared-migration-utils/generate_pydantic_schema.py --source {ts_file}` to extract typescript interfaces and output clean Pydantic V2 schemas.
...
2. Run the pydantic translator helper:
   ```bash
   python3 ../shared-migration-utils/generate_pydantic_schema.py --source {path_to_api_file}
   ```
